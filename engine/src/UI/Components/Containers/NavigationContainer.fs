...
module NavigationContainer =
    ...
    type private GridSwitchItem<'T when 'T :> Widget> =
        {
            Widget: 'T
            X: int
            Y: int
    ...
    type Grid<'T when 'T :> Widget>() as this =
        ...
        member private this.WhoIsFocused : int option = Seq.tryFindIndex (fun (c: GridSwitchItem<'T>) -> c.Widget.Focused) children
        member private this.WhoShouldFocus =
            if children.Count = 0 then failwithf "Tried to focus this %O with no children" this
            if last_selected >= children.Count then last_selected <- 0
        ...
        member private this.Up() =
            ...
            | Some i ->
                ...
                let mutable found = Seq.tryFindIndex(fun (item: GridSwitchItem<'T>) -> item.X = c.X && item.Y = p && item.Widget.Focusable) children
                while found.IsNone && p <> c.Y do
                    p <- (p - 1) %% rows
                    found <- Seq.tryFindIndex(fun (item: GridSwitchItem<'T>) -> item.X = c.X && item.Y = p && item.Widget.Focusable) children
                match found with
                | Some i -> 
                    last_selected <- i
        ...
        member private this.Left() =
            ...
            | Some i ->
                ...
                let mutable found = Seq.tryFindIndex(fun (item: GridSwitchItem<'T>) -> item.X = p && item.Y = c.Y && item.Widget.Focusable) children
                while found.IsNone && p <> c.X do
                    p <- (p - 1) %% columns
                    found <- Seq.tryFindIndex(fun (item: GridSwitchItem<'T>) -> item.X = p && item.Y = c.Y && item.Widget.Focusable) children
                match found with
                | Some i -> 
                    last_selected <- i
                    children.[i].Widget.Focus()
        ...
        member private this.CanUp() =
            ...
            | Some i ->
                let c = children.[i]
                children.Any(fun (item: GridSwitchItem<'T>) -> item.X = c.X && item.Y < c.Y && item.Widget.Focusable)
            | None -> false
        ...
        override this.Draw() =
            for { Widget = c } in children do
                if this.Floating || c.VisibleBounds.Visible then c.Draw()
        
        member this.Add (child: 'T, x: int, y: int) =
            if x < 0 || y < 0 then failwith "X and Y can't be negative"
            rows <- max rows (y + 1)

module Prelude.Gameplay.Difficulty
...
let getLayoutInfo keycount l: LayoutInfo option =
...
type RatingReport(notes : TimeData<NoteRow>, rate : float, layout, keys) =
    let layoutData =
        ...
        | Some l -> l
        | None -> getAvailableLayouts keys |> List.head |> getLayoutInfo keys |> extractOption
...
let performanceMetric (rr : RatingReport) (chart : Prelude.Charts.Interlude.Chart) =
    ScoreMetric<PerformanceMetricState>(
        "Interlude",
        (0.0, 0.01, Array.zeroCreate chart.Keys, 0.01, Array.zeroCreate chart.Keys),

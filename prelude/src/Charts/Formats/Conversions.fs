...
module ``osu!`` =
    ...
    let private convertTimingPoints (points: TimingPoint list) (endTime: Time) : TimeArray<BPM> * TimeArray<float32> =
        ...
        let mostCommonBpm =
            (bpmDurations points).OrderByDescending(fun p -> p.Value).First().Key

        let addOrSkipSV (offset, value) sv =
            match sv with
            | [] -> [ { Time = offset; Data = value } ]
            | { Time = time; Data = oldValue } :: s ->
                if oldValue = value then
                    sv
                else
                    { Time = offset; Data = value } :: sv
        ...
        let (bpm, sv, _) =
            let func
                ...
                | (TimingPoint.SV(offset, value, _, _)) -> bpm, addOrSkipSV (offset, (value * scroll)) sv, scroll

            List.fold func ([], [], 1.0f) points

        bpm |> Array.ofList |> Array.rev, sv |> Array.ofList |> Array.rev

    let private rateRegex =
        Regex(
    ...
    let toInterlude (b: Beatmap) (action: ConversionAction) : Chart =
        ...
        if snaps.Length = 0 then
            failwith "osu! chart has no notes"
    ...
    let toInterlude (sm: StepmaniaData) (action: ConversionAction) =
        ...
        let convert_difficulty (i: int) (diff: ChartData) : Chart =
            ...
            let header =
                {
                    Title = title
                    TitleNative =
                        match metadataFallbackOpt [ sm.TITLETRANSLIT ] with
                        | Some t when t = title -> None
                        | x -> x
                    Artist = artist
                    ArtistNative =
                        match metadataFallbackOpt [ sm.ARTISTTRANSLIT ] with
            ...
            if notes.Length = 0 then
                failwith "Stepmania chart has no notes"
...
module Interlude =
    ...
    let private convertSnapsToHitobjects (snaps: TimeArray<NoteRow>) keys =
        ...
        convert (snaps |> Array.toList) |> List.ofSeq
        ...
        let tps =
            seq {
                ...
                if List.isEmpty bs then
                    ...
                    let mostCommonMspb =
                        (bpmDurations bs endTime).OrderByDescending(fun p -> p.Value).First().Key
        ...
        | ".osu" ->
            try
                ...
                then
                    ...
                    []
            with err ->
                Logging.Debug(sprintf "Error loading %s" action.Source, err)
                []
        | _ -> []

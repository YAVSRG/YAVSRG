...
module PersonalBests =
    ...
    let rec get_best_above_with_rate (minimum_rate: float32) (bests: PersonalBests<'T>) =
        ...
        | [] -> None
        | (value, rate) :: xs -> 
            if rate = minimum_rate then Some (value, rate)
            elif rate < minimum_rate then None
            else get_best_above_with_rate minimum_rate xs |> function None -> Some (value, rate) | Some x -> Some x

    let get_best_above minimum_rate = get_best_above_with_rate minimum_rate >> Option.map fst

    let rec get_best_below_with_rate (maximum_rate: float32) (bests: PersonalBests<'T>) =
        match bests with
        | [] -> None
        | (value, rate) :: xs ->
            if rate > maximum_rate then get_best_below_with_rate maximum_rate xs
            else Some (value, rate)
            
    let get_best_below maximum_rate = get_best_below_with_rate maximum_rate >> Option.map fst

...
type HitObject =
    ...
    static member inline CreateManiaNote(keys: int, key: int, time: ^T) : HitObject = HitCircle.Create(keys, key, time) |> HitCircle
    
    static member inline CreateManiaHold(keys: int, key: int, start_time: ^T, end_time: ^T) : HitObject = Hold.Create(keys, key, start_time, end_time) |> Hold

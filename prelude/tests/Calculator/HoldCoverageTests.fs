...
module HoldCoverageTests =
    ...
    let NotesHaveNoHolds () =
        ...
        let note_data = NotesBuilder(2).Note(0.0f<ms>, 0).Note(100.0f<ms>, 1).Build()
    ...
    let NotesHaveOneHold () =
        ...
        let note_data =
            NotesBuilder(2).HoldUntil(0.0f<ms>, 200.0f<ms>, 0).Note(200.0f<ms>, 1).Build()
    ...
    let NotesDuringBody () =
        ...
        let notes =
            [|
                ...
                {
                    Time = 1000.0f<ms> + HoldCoverage.SHORTEN_AMOUNT * 1.0f<rate>
                    Data = [| NoteType.HOLDTAIL; NoteType.NOTHING |]
                }
            |]

        let output =
            HoldCoverage.calculate_coverage({ Keys = 2; Notes = notes }, 1.0f<rate>)

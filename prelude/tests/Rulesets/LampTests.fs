...
module LampTests =
    ...
    let LAMPS =
        [|
            { Name = "SDCB"; Requirement = LampRequirement.ComboBreaksAtMost(9); Color = Color.FromArgb(255, 160, 160) }
            { Name = "1CB"; Requirement = LampRequirement.ComboBreaksAtMost(1); Color = Color.FromArgb(160, 160, 160) }
            { Name = "FC"; Requirement = LampRequirement.ComboBreaksAtMost(0); Color = Color.FromArgb(80, 255, 80) }
            { Name = "SDG"; Requirement = LampRequirement.JudgementAtMost(2, 9); Color = Color.FromArgb(160, 255, 160) }
            { Name = "1G"; Requirement = LampRequirement.JudgementAtMost(2, 1); Color = Color.FromArgb(200, 160, 255) }
            { Name = "PFC"; Requirement = LampRequirement.JudgementAtMost(2, 0); Color = Color.FromArgb(255, 255, 80) }
            { Name = "MFC"; Requirement = LampRequirement.JudgementAtMost(1, 0); Color = Color.FromArgb(160, 255, 255) }
        |]

    [<Test>]
    let LampTestData_Valid () =
        for a, b in Seq.pairwise LAMPS do
            if not(b.Requirement.IsStricterThan(a.Requirement)) then
                Assert.Fail()

...
module ScoreScreenStatsTests =
    ...
    let TEST_SCORE =
        ...
        let chart =
            (Osu_To_Interlude.convert
                beatmap
                { Config = ConversionOptions.Pack("osu!", None, LinkAssetFiles); Source = beatmap_path }
             |> expect)
                .Chart
    ...
    let ColumnFilter_TwoNotes () =
        ...
        let note_data =
            NotesBuilder(4).Note(0.0f<ms>).Note(1000.0f<ms>).Note(2000.0f<ms>, 2).Note(3000.0f<ms>, 2).Build()

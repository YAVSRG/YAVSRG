    ...
    let load_stepmania_packs () =
        async {
            ...
            | Some(d: {| data: ResizeArray<EOPack> |}) ->
                for p in d.data do
                    if packs.Stepmania.ContainsKey(p.id) then
                        packs.Stepmania.Remove(p.id) |> ignore
                    ...
                    packs.Stepmania.Add(
                        p.id,
                        {
                            ...
                            Mirrors =
                                [
                                    Archive.DownloadUrl.create p.attributes.download
                                    Archive.DownloadUrl.create p.attributes.mirror
                                ]
                                |> List.distinct

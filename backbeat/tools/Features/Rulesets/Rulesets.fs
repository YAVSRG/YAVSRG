...
module Rulesets =
    ...
    let defaultRulesets =
        ...
        @ List.map (fun j -> (sprintf "sc-j%i" j, PrefabRulesets.SC.create j)) [ 1; 2; 3; 4; 5; 6; 7; 8; 9 ]
        @ List.map (fun j -> (sprintf "wife-j%i" j, PrefabRulesets.Wife.create j)) [ 1; 2; 3; 4; 5; 6; 7; 8; 9 ]
        @ List.map

...
module Rulesets =
    ...
    let defaultRulesets =
        ...
        @ List.map
            (fun j -> (sprintf "sc-j%i" j, PrefabRulesets.SC.create j))
            [1; 2; 3; 4; 5; 6; 7; 8; 9]
        @ List.map
            (fun j -> (sprintf "wife-j%i" j, PrefabRulesets.Wife.create j))
            [1; 2; 3; 4; 5; 6; 7; 8; 9]
        @ List.map
            (fun (d: PrefabRulesets.Ex_Score.Type) -> (sprintf "xs-%s" (d.Name.ToLower()), PrefabRulesets.Ex_Score.create d))
            [PrefabRulesets.Ex_Score.mizu]

    let main_rulesets = ["osu-od-5"; "osu-od-8"; "sc-j4"; "sc-j5"; "wife-j4"; "xs-sdvx"]

    let compile_rulesets() =
        let data : PrefabRulesets.Repo = { Rulesets = Dictionary<string, Ruleset>() }
        ...
        for file in Directory.GetFiles(RULESETS_PATH) do
            if file.EndsWith(".ruleset") then
                ...
                let d = match JSON.FromFile<Ruleset> file with Ok d -> d | Error e -> failwithf "Error loading '%s': %A" f e;

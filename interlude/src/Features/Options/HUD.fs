    ...
    let position_editor (setting: Setting<HUDPosition>) (default_pos: HUDPosition) =
        ...
        |+ PageSetting(
            ...
            Selector<_>
                .FromBool(
                    Setting.make
                        (fun v -> setting.Set { setting.Value with RelativeToPlayfield = not v })
        ...
        |+ PageButton(
            "hud.generic.reset",
            fun () ->
                setting.Value <- default_pos
        )
            .Pos(450.0f)
            .Tooltip(Tooltip.Info("hud.generic.reset"))

type EditAccuracyPage() as this =
    inherit Page()
...
type EditTimingDisplayPage() as this =
    inherit Page()
    ...
    let thickness = Setting.simple user_options.TimingDisplayThickness |> Setting.bound 1.0f 25.0f

    let release_thickness =
        Setting.simple user_options.TimingDisplayReleasesExtraHeight |> Setting.bound 0.0f 20.0f

    let half_scale_releases = Setting.simple user_options.TimingDisplayHalfScaleReleases

    let animation_time =
        Setting.simple user_options.TimingDisplayFadeTime |> Setting.bound 100.0f 2000.0f
...
type EditComboPage() as this =
    ...
    let pop_amount = Setting.simple noteskin_options.ComboPop |> Setting.bound 0.0f 20.0f
    let growth_amount = Setting.simple noteskin_options.ComboGrowth |> Setting.bound 0.0f 0.05f
...
type EditJudgementCounterPage() as this =
    inherit Page()
    ...
    let animation_time = Setting.simple user_options.JudgementCounterFadeTime |> Setting.bound 100.0 1000.0
...
type EditJudgementMeterPage() as this =
    ...
    let ignore_perfect_judgements = Setting.simple user_options.JudgementMeterIgnorePerfect
    let prioritise_lower_judgements = Setting.simple user_options.JudgementMeterPrioritiseLower

    let animation_time = Setting.simple user_options.JudgementMeterFadeTime |> Setting.bound 100.0f 2000.0f
...
type EditEarlyLateMeterPage() as this =
    ...
    let animation_time =
        Setting.simple user_options.EarlyLateMeterFadeTime |> Setting.bound 100.0f 2000.0f
...
type EditHUDPage() as this =
    ...
    do
        this.Content(
            grid
            |+ PageButton("hud.accuracymeter", (fun () -> Menu.ShowPage EditAccuracyPage))
                .Tooltip(Tooltip.Info("hud.accuracymeter"))
            |+ PageButton("hud.hitmeter", (fun () -> Menu.ShowPage EditTimingDisplayPage))
                .Tooltip(Tooltip.Info("hud.hitmeter"))
            |+ PageButton("hud.combo", (fun () -> Menu.ShowPage EditComboPage))
            ...
            |+ PageButton("hud.judgementcounts", (fun () -> Menu.ShowPage EditJudgementCounterPage))

...
type HudGraph (config: HudConfig, state: PlayState) =
    ...
    override this.Update(elapsed_ms, moved) =
        ...
        while hits.Count > 0 && (float32 now - float32 hits.[0].Time) * config.HudGraphScrollSpeed * 0.01f > this.Bounds.Width do
            hits.RemoveAt(0)
            printfn "d"
    ...
    override this.Draw() =
        ...
        let hit_opacity = int config.HudGraphWindowOpacity
        this.DrawWindows hit_opacity
        let speed = config.HudGraphScrollSpeed
        let now = state.CurrentChartTime()

        let r =
            fun p1 p2 ->
                let scaledreverseage = (float32 now - p2) * speed * 0.01f
                ...
                Rect.FromEdges(
                    ...
                    this.Bounds.Top,
                    this.Bounds.Bottom),
                    this.Bounds.Right - scaledreverseage,               
                    System.Math.Clamp(this.Bounds.CenterY + p1 * this.Bounds.Height / float32 MAX_WINDOW / 2f - size * 0.5f,
                    this.Bounds.Top,
                    this.Bounds.Bottom))
        ...
        for hit in hits do
            ...
            if (float32 now - float32 hit.Time) * speed * 0.01f < this.Bounds.Width then
                Render.rect
                    rect
                    colorWithOpacity
        ...
        if config.HudGraphShowIncomingNotes then
            ...
            for { Time = time; Data = notes } in state.WithColors.Source.Notes do
                let window_time = float32 (first_note + now - time)

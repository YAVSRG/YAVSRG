...
type ScoreGraphSettingsPage(graph: ScoreGraph) =
    ...
    let column_filter_ui, _ = refreshable_row (fun () -> graph.Keys) (fun k _ -> Checkbox(column_filter_setting k, Position = Position.SliceLeft(50.0f).Translate(float32 k * 100.0f, 0.0f)))

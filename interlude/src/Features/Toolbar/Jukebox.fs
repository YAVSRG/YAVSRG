...
type Jukebox() =
    ...
    override this.Init(parent) =
        ...
        |+ Button(Icons.SKIP_BACK,
            ...
            Disabled = (fun () -> Screen.current_type = ScreenType.Lobby || not (LevelSelect.History.can_go_back()))).Position(Position.Shrink(5.0f).SliceL(45.0f))
        |+ Button(Icons.PAUSE,
            (fun () -> if Song.playing() then Song.pause() else Song.resume()),
            Hotkey = "pause_music").Position(Position.Shrink(5.0f).SliceL(45.0f).Translate(45.0f, 0.0f))
        ...
        |* Button(Icons.SKIP_FORWARD,
            ...
            Disabled = (fun () -> Screen.current_type = ScreenType.Lobby)).Position(Position.Shrink(5.0f).SliceL(45.0f).Translate(90.0f, 0.0f))
        base.Init parent
